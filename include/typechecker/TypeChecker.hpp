...
namespace zust {
    ...
    static const std::set<std::string> integral_types = {"integer",  "size_t", "uint8_t", "uint16_t", "uint32_t",
    ...
    public:
        ...
        bool shouldCodegen() { return shouldCodegen_; }

        static inline std::string typeName(const TypeInfo &info) { return info.name; }

        static inline TypeInfo promoteType(const TypeInfo &a, const TypeInfo &b) {
            if (a.isString || b.isString || a.isPointer || b.isPointer || a.isUserDefined || b.isUserDefined) {
                throw std::runtime_error("Invalid type promotion between: " + typeName(a) + " and " + typeName(b));
            }
