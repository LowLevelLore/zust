...
namespace zust {
    ...
    protected:
        ...
        virtual void emitEpilogue(std::shared_ptr<ScopeContext> scope, std::ostringstream &out,
                                  bool clearRax = false) = 0;
        ...
        virtual std::string generateFunctionCall(std::unique_ptr<ASTNode> node,
                                                 std::ostringstream &out) = 0;  // Expression
        virtual void generateFunctionDeclaration(std::unique_ptr<ASTNode> node, std::ostringstream &out,
                                                 bool force = false) = 0;  // Statement
        virtual void generateExternFunctionDeclaration(std::unique_ptr<ASTNode> node,
                                                       std::ostringstream &out) = 0;  // Statement

    public:
        CodeGen(RegisterAllocator alloc, std::ostream &outstream)
            : stringLabelCount(0),
              blockLabelCount(0),
              doubleLabelCount(0),
              floatLabelCount(0),
